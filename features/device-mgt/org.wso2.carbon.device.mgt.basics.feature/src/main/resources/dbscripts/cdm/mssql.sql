...
CREATE TABLE DM_DEVICE_HIERARCHY (
  ...
  PRIMARY KEY (ID),
  CONSTRAINT fk_DM_DEVICE_HIERARCHY FOREIGN KEY (DEVICE_ID) REFERENCES
    DM_DEVICE (DEVICE_IDENTIFICATION) ON DELETE CASCADE ON UPDATE CASCADE
);

IF NOT EXISTS(SELECT * FROM SYS.INDEXES WHERE NAME = 'IDX_DEVICE_ID' AND OBJECT_ID = OBJECT_ID('DM_DEVICE_HIERARCHY'))
CREATE INDEX IDX_DEVICE_ID ON DM_DEVICE_HIERARCHY(DEVICE_ID);
