...
$(document).ready(function () {
    ...
    $("#add-profile").click(function () {
        $(modalPopupContent).html($('#add-profile-window').html());
        showPopup();

        var checkboxes = document.querySelectorAll('input.subOption');
        var checkall = document.getElementById('skip');
        ...
        $("a#add-profile-link").click(function () {
            var profileName = $("#profile-name").val();
            var url = $("#url").val();
            var isSupervised = document.querySelector("#is-supervised").checked;
            var mdmRemovable = document.querySelector("#mdm-removable").checked;
            var isMandatory = document.querySelector("#is-mandatory").checked;
            var department  = $("#department").val();
            var orgMagic = Math.floor((Math.random() * 1800000000000) + 1000000000000);
            var supportPhone = $("#sup-phone").val();
            var supportEmail = $("#sup-mail").val();
            var anchorCerts = document.getElementById("anchor-certs").value;
            var appleIdCheck = document.querySelector("#id-check").checked;
            var biometricCheck = document.querySelector("#biometric-check").checked;
            var displayToneCheck = document.querySelector("#display-tone-check").checked;
            var locationCheck = document.querySelector("#location-check").checked;
            var passcodeCheck = document.querySelector("#passcode-check").checked;
            var paymentCheck = document.querySelector("#payment-check").checked;
            var restoreCheck = document.querySelector("#restore-check").checked;
            var siriCheck = document.querySelector("#siri-check").checked;
            var tosCheck = document.querySelector("#tos-check").checked;
            var zoomCheck = document.querySelector("#zoom-check").checked;
            var homeSensitivityCheck = document.querySelector("#homeSensitivity-check").checked;
            var boardingCheck = document.querySelector("#boarding-check").checked;
            var watchMigrationCheck = document.querySelector("#watchMigration-check").checked;

            var newProfilePayload = {};
            var skip = {};
            newProfilePayload["profile_name"] = unescape((profileName));
            newProfilePayload["url"] = unescape((url));
            newProfilePayload["is_supervised"] = unescape((isSupervised));
            newProfilePayload["is_mandatory"] = unescape((isMandatory));
            newProfilePayload["is_mdm_removable"] = unescape((mdmRemovable));
            newProfilePayload["department"] = unescape((department));
            newProfilePayload["org_magic"] = unescape((orgMagic));
            newProfilePayload["support_phone_number"] = unescape((supportPhone));
            newProfilePayload["support_email_address"] = unescape((supportEmail));
            skip["AppleID"] = unescape((appleIdCheck));
            skip["Biometric"] = unescape((biometricCheck));
            newProfilePayload["skip_setup_items"] = unescape((skip));

            console.log(newProfilePayload);
        });
    });
